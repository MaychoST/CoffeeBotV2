# handlers/admin_menu_management_handler.py
...
from keyboards import (
    get_admin_menu_management_keyboard, get_admin_categories_management_keyboard,
    get_admin_select_category_for_items_keyboard, get_admin_items_management_keyboard,
    get_confirm_delete_item_keyboard, get_fsm_navigation_keyboard,
    get_confirm_delete_category_keyboard, get_admin_item_prices_management_keyboard,
    get_confirm_delete_price_keyboard, get_admin_menu_keyboard,
    get_confirm_add_another_price_keyboard
)
from database import (
    get_all_menu_categories, add_menu_category, delete_menu_category, update_menu_category, get_menu_category_by_id,
    add_menu_item, get_menu_items_by_category_id, get_menu_item_by_id, update_menu_item, delete_menu_item,
    add_menu_item_price, get_prices_for_menu_item, delete_menu_item_price,
    update_menu_item_price, get_menu_item_price_by_id,
...
# ... (вспомогательные функции остаются без изменений, я их скрою) ...
...
async def show_categories_management_menu(target: Message | CallbackQuery, edit_message: bool = False):
    categories = await get_all_menu_categories(only_active=False)
...
async def show_select_category_for_items_menu(target: Message | CallbackQuery, edit_message: bool = False):
...
async def show_items_management_menu(target: Message | CallbackQuery, category_id: int, edit_message: bool = False):
    category = await get_menu_category_by_id(category_id)
    if not category: await show_select_category_for_items_menu(target, edit_message=edit_message); return
    items = await get_menu_items_by_category_id(category_id, only_active=False)
...
async def show_item_prices_management_menu(target: Message | CallbackQuery, item_id: int, edit_message: bool = False):
    item = await get_menu_item_by_id(item_id)
    if not item: await show_select_category_for_items_menu(target, edit_message=True); return
    prices = await get_prices_for_menu_item(item_id)
...
async def admin_manage_categories_entry(message: Message, state: FSMContext):
    if not await check_admin_auth(message, state): return
    await state.set_state(None)
    await show_categories_management_menu(message)


@router.message(F.text == MANAGE_ITEMS_TEXT, StateFilter(AdminNavigationStates.in_menu_management))
async def admin_manage_items_entry(message: Message, state: FSMContext):
    if not await check_admin_auth(message, state): return
    await state.set_state(None)
    await show_select_category_for_items_menu(message)
...
async def cq_admin_menu_manage_back(cq: CallbackQuery, state: FSMContext):
    # Сначала проверяем права, пока state еще не изменен
    if not await check_admin_auth(cq, state):
        return

    # --- НАЧАЛО ИСПРАВЛЕНИЯ ---
    # Применяем наш безопасный паттерн
    auth_data = await state.get_data()
    await state.clear()
    if 'role' in auth_data:
        await state.set_data({'role': auth_data['role']})
    # --- КОНЕЦ ИСПРАВЛЕНИЯ ---

    # Теперь, когда состояние чистое, устанавливаем навигационное состояние
    await state.set_state(AdminNavigationStates.in_menu_management)

    # Отправляем новое сообщение, удалив старое
    if cq.message:
...
async def cq_admin_item_cat_select_back(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    await show_select_category_for_items_menu(cq, edit_message=True)


@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_ITEM_MANAGE_CAT_SELECT))
async def cq_admin_item_manage_cat_selected(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    category_id = int(cq.data.split(":")[1]);
    await show_items_management_menu(cq, category_id, edit_message=True)


@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_ITEM_MANAGE_PRICES))
async def cq_admin_item_manage_prices_entry(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    item_id = int(cq.data.split(":")[1]);
    await show_item_prices_management_menu(cq, item_id, edit_message=True)
...
async def process_new_category_name(message: Message, state: FSMContext):
    ...
    if not new_name or await check_category_name_exists(new_name):
    ...
    await add_menu_category(name=new_name)
...
# ... (остальные хендлеры категорий остаются без изменений) ...
@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_CAT_EDIT))
async def cq_admin_cat_edit_start(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    category_id = int(cq.data.split(":")[1]);
    category = await get_menu_category_by_id(category_id)
...
async def process_edit_category_name(message: Message, state: FSMContext):
    ...
    elif await check_category_name_exists(new_name, category_id_to_exclude=cat_id):
...
async def cq_admin_cat_toggle_active(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    cat_id = int(cq.data.split(":")[1]);
    category = await get_menu_category_by_id(cat_id)
    if not category: await cq.answer("Категория не найдена!", True); return
    new_active = not category['is_active'];
    await update_menu_category(cat_id, is_active=new_active)
    await cq.answer(f"Категория '{category['name']}' {'скрыта' if not new_active else 'активирована'}.");
    await show_categories_management_menu(cq, edit_message=True)


@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_CAT_DELETE_PROMPT))
async def cq_admin_cat_delete_prompt(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    cat_id = int(cq.data.split(":")[1]);
    category = await get_menu_category_by_id(cat_id)
    if not category: await cq.answer("Категория не найдена!", True); return
...
async def cq_admin_cat_confirm_delete_action(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    cat_id = int(cq.data.split(":")[1]);
    cat = await get_menu_category_by_id(cat_id)
    await delete_menu_category(cat_id);
...
async def cq_admin_cat_cancel_delete_action(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return; await show_categories_management_menu(cq, edit_message=True)


# --- FSM для Товаров ---
@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_ITEM_ADD_NEW))
async def item_creation_start(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    category_id = int(cq.data.split(":")[1]);
    category = await get_menu_category_by_id(category_id)
    if not category: await cq.answer("Категория не найдена.", True); return
...
# --- ИСПРАВЛЕНИЕ БАГА №2: Убираем шаг с описанием ---
@router.message(ItemCreationStates.waiting_for_item_name, F.text)
async def item_creation_process_name(message: Message, state: FSMContext):
    item_name = message.text.strip();
    fsm_data = await state.get_data();
    category_id = fsm_data.get("item_creation_category_id")
    if not item_name or await check_item_name_exists(category_id, item_name):
    ...
    await message.answer(
        ...
        reply_markup=get_fsm_navigation_keyboard(
            cancel_callback=CB_PREFIX_ADMIN_ITEM_CREATION_CANCEL_PRICE,
            back_callback=CB_ADMIN_FSM_BACK
        ),
        parse_mode="HTML")


# --- ИЗМЕНЕНИЕ: item_creation_process_description удален ---
...
async def item_creation_process_price_option_name(message: Message, state: FSMContext):
    ...
    await message.answer("Введите значение цены (например, 150):",
                         reply_markup=get_fsm_navigation_keyboard(
                             cancel_callback=CB_PREFIX_ADMIN_ITEM_CREATION_CANCEL_PRICE,
                             back_callback=CB_ADMIN_FSM_BACK
                         ))
...
async def item_creation_process_price_value(message: Message, state: FSMContext):
    try:
    ...
    if not item_id:
        # --- ИЗМЕНЕНИЕ: Убрали передачу description ---
        item_id = await add_menu_item(category_id=fsm_data["item_creation_category_id"],
    ...
    await add_menu_item_price(item_id=item_id, option_name=fsm_data.get("current_price_option_name_for_creation"),
                              price=price_value)
...
async def item_creation_finish(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    fsm_data = await state.get_data();
    category_id = fsm_data.get("item_creation_category_id")
    ...
    if cq.message: await cq.message.delete(); await cq.message.answer("✅ Товар успешно создан!",
                                                                      reply_markup=ReplyKeyboardRemove())
    if category_id: await show_items_management_menu(cq.message, category_id)


# ... (все остальные хендлеры остаются без изменений, я их скрою для краткости) ...
@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_ITEM_EDIT_INFO))
async def cq_admin_item_edit_info_start(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    item_id = int(cq.data.split(":")[1]);
    item = await get_menu_item_by_id(item_id)
...
async def process_edit_item_name(message: Message, state: FSMContext):
    name_in = message.text.strip();
    data = await state.get_data();
    old_name = data.get("old_item_name");
    final_name = old_name
    if name_in and name_in != '-':
        if await check_item_name_exists(data["editing_item_category_id"], name_in, data["editing_item_id"]):
...
async def process_edit_item_description_and_save(message: Message, state: FSMContext):
    ...
    await update_menu_item(item_id=item_id, name=name_save, description=final_desc);
    ...
    await show_items_management_menu(message, cat_id_ret)


@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_ITEM_TOGGLE_ACTIVE))
async def cq_admin_item_toggle_active(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    item_id = int(cq.data.split(":")[1]);
    item = await get_menu_item_by_id(item_id)
    if not item: await cq.answer("Товар не найден!", True); return
    new_active = not item['is_active'];
    await update_menu_item(item_id, is_active=new_active)
    await cq.answer(f"Товар '{item['name']}' {'скрыт' if not new_active else 'активирован'}.");
    await show_items_management_menu(cq, item['category_id'], edit_message=True)


@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_ITEM_DELETE_PROMPT))
async def cq_admin_item_delete_prompt(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    item_id = int(cq.data.split(":")[1]);
    item = await get_menu_item_by_id(item_id)
    if not item: await cq.answer("Товар не найден!", True); return
...
async def cq_admin_item_confirm_delete_action(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    parts = cq.data.split(":");
    item_id, cat_id_ret = int(parts[1]), int(parts[2])
    item_log = await get_menu_item_by_id(item_id);
    ...
    await show_items_management_menu(cq, cat_id_ret, edit_message=True)


@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_ITEM_CANCEL_DELETE))
async def cq_admin_item_cancel_delete_action(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
...
async def cq_admin_price_add_new_to_existing_item_start(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    item_id = int(cq.data.split(":")[1]);
    item = await get_menu_item_by_id(item_id)
    if not item: await cq.answer("Товар не найден.", True); return
...
async def process_new_price_value_for_existing_and_save(message: Message, state: FSMContext):
    try:
    ...
    await add_menu_item_price(item_id=item_id, price=price_val, option_name=opt_name);
    ...
    await show_item_prices_management_menu(message, item_id)


@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_PRICE_EDIT))
async def cq_admin_price_edit_start(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    price_id = int(cq.data.split(":")[1]);
    price_entry = await get_menu_item_price_by_id(price_id)
    if not price_entry: await cq.answer("Запись цены не найдена!", True); return
    item = await get_menu_item_by_id(price_entry['item_id'])
...
async def process_edit_price_option_name(message: Message, state: FSMContext):
    ...
    await message.answer(f"Имя опции будет: {new_opt_disp}.\nНовое значение цены ('-' оставить):",
                         reply_markup=get_fsm_navigation_keyboard(cancel_callback=CB_PREFIX_ADMIN_PRICE_EDIT_CANCEL,
                                                                  back_callback=CB_ADMIN_FSM_BACK), parse_mode="HTML")


@router.message(PriceManagementStates.waiting_for_edit_price_value, F.text)
async def process_edit_price_value_and_save(message: Message, state: FSMContext):
    ...
    if not (price_param is not None or opt_param is not None or set_opt_null_param):
        ...
        await update_menu_item_price(price_id=price_id_edit, new_price=price_param, new_option_name=opt_param,
    ...
    await show_item_prices_management_menu(message, item_id_ret)


@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_PRICE_DELETE_PROMPT))
async def cq_admin_price_delete_prompt(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    price_id = int(cq.data.split(":")[1]);
    price_entry = await get_menu_item_price_by_id(price_id)
    if not price_entry: await cq.answer("Запись цены не найдена!", True); return
...
async def cq_admin_price_confirm_delete_action(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    parts = cq.data.split(":");
    price_id, item_id_ret = int(parts[1]), int(parts[2])
    await delete_menu_item_price(price_id);
    await cq.answer("Цена/опция удалена.", show_alert=True)
    await show_item_prices_management_menu(cq, item_id_ret, edit_message=True)


@router.callback_query(F.data.startswith(CB_PREFIX_ADMIN_PRICE_CANCEL_DELETE))
async def cq_admin_price_cancel_delete_action(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    item_id_ret = int(cq.data.split(":")[1]);
    await show_item_prices_management_menu(cq, item_id_ret, edit_message=True)


async def cancel_fsm_and_show_menu(cq: CallbackQuery, state: FSMContext, show_menu_func, **kwargs):
    auth_data = await state.get_data();
    await state.clear()
    if 'role' in auth_data: await state.update_data(role=auth_data['role'])
    await cq.answer("Действие отменено.")
    if cq.message: await cq.message.delete(); await show_menu_func(cq.message, **kwargs)


@router.callback_query(F.data.in_({CB_PREFIX_ADMIN_CAT_CREATION_CANCEL, CB_PREFIX_ADMIN_CAT_EDIT_CANCEL}),
                       StateFilter("*"))
async def cancel_category_fsm(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    await cancel_fsm_and_show_menu(cq, state, show_categories_management_menu)
...
async def item_creation_cancel_fsm(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    fsm_data = await state.get_data();
    category_id = fsm_data.get("item_creation_category_id")
    show_func = show_items_management_menu if category_id else show_select_category_for_items_menu
    await cancel_fsm_and_show_menu(cq, state, show_func, **({"category_id": category_id} if category_id else {}))


@router.callback_query(StateFilter(ItemInfoEditStates), F.data == CB_PREFIX_ADMIN_ITEM_INFO_EDIT_CANCEL)
async def cancel_item_info_edit_fsm(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    fsm_data = await state.get_data();
    category_id = fsm_data.get("editing_item_category_id")
    await cancel_fsm_and_show_menu(cq, state, show_items_management_menu, category_id=category_id)
...
async def cancel_price_fsm(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    fsm_data = await state.get_data();
    item_id = fsm_data.get("add_price_to_existing_item_id") or fsm_data.get("editing_price_item_id")
    await cancel_fsm_and_show_menu(cq, state, show_item_prices_management_menu, item_id=item_id)


@router.callback_query(F.data == CB_ADMIN_FSM_BACK, StateFilter("*"))
async def fsm_go_back(cq: CallbackQuery, state: FSMContext):
    if not await check_admin_auth(cq, state): return
    current_state, fsm_data = await state.get_state(), await state.get_data()
    # --- ИЗМЕНЕНИЕ: Упрощенная логика возврата ---
    if current_state == ItemCreationStates.waiting_for_price_option_name:
        ...
        markup = get_fsm_navigation_keyboard(cancel_callback=CB_PREFIX_ADMIN_ITEM_CREATION_CANCEL_PRICE,
                                             back_callback=CB_ADMIN_FSM_BACK)
        await _safe_edit_or_send(cq, text, markup, edit=True, parse_mode="HTML")
    elif current_state == ItemInfoEditStates.waiting_for_edit_item_description:
        await state.set_state(ItemInfoEditStates.waiting_for_edit_item_name)
        text = f"Редактирование <b>{html.quote(fsm_data.get('old_item_name', ''))}</b>\nВведите новое название ('-' оставить):"
        markup = get_fsm_navigation_keyboard(cancel_callback=CB_PREFIX_ADMIN_ITEM_INFO_EDIT_CANCEL)
        await _safe_edit_or_send(cq, text, markup, edit=True, parse_mode="HTML")
    elif current_state == PriceManagementStates.waiting_for_new_price_value:
        await state.set_state(PriceManagementStates.waiting_for_new_price_option_name)
        text = f"Добавление цены для <b>{html.quote(fsm_data.get('add_price_to_existing_item_name', ''))}</b>\nНазвание опции ('-' если нет):"
        markup = get_fsm_navigation_keyboard(cancel_callback=CB_PREFIX_ADMIN_PRICE_TO_EXISTING_CANCEL)
        await _safe_edit_or_send(cq, text, markup, edit=True, parse_mode="HTML")
    elif current_state == PriceManagementStates.waiting_for_edit_price_value:
        await state.set_state(PriceManagementStates.waiting_for_edit_price_option_name)
        item_name, curr_opt, curr_price = fsm_data.get("editing_item_name",
                                                       "???"), f"'{html.quote(str(fsm_data.get('old_price_option_name')))}'" if fsm_data.get(
            'old_price_option_name') else "нет", f"{float(fsm_data.get('old_price_value', 0)):.2f}"
        text = f"Редактирование цены для <b>{html.quote(item_name)}</b>\nТек. опция: {curr_opt}, тек. цена: {curr_price}\n\nНовое название опции ('-' оставить, 'пусто' удалить):"
        markup = get_fsm_navigation_keyboard(cancel_callback=CB_PREFIX_ADMIN_PRICE_EDIT_CANCEL)
        await _safe_edit_or_send(cq, text, markup, edit=True, parse_mode="HTML")
    else:
...
async def show_service_info(cq: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await cq.answer(
            "Не удалось отобразить сервисную информацию.", show_alert=True)
