...
from constants import (
    CURRENCY_SYMBOL, VIEW_ACTIVE_ORDERS_TEXT,
    CB_PREFIX_COMPLETE_ORDER, CB_PREFIX_EDIT_ORDER, CB_PREFIX_EDIT_ORDER_DELETE_PROMPT,
    CB_PREFIX_EDIT_ORDER_CONFIRM_DELETE, CB_PREFIX_EDIT_ORDER_ADD_ITEM_START,
    CB_PREFIX_EDIT_ORDER_FINISH
)
from utils import _display_active_orders_list, _display_edit_order_interface
from database import (
    get_order_by_id, delete_order_item, recalculate_order_total_amount,
    delete_order, update_order_status, get_order_items, get_all_menu_categories
)
from keyboards import (
    get_edit_order_actions_keyboard,
    get_items_to_delete_keyboard,
...
async def check_auth(target: Message | CallbackQuery, state: FSMContext) -> str | None:
    """Проверяет авторизацию пользователя и возвращает его роль или None."""
    data = await state.get_data()
    role = data.get('role')
    if role not in ['admin', 'barista']:
        if isinstance(target, CallbackQuery):
            await target.answer("Доступ запрещен. Пожалуйста, авторизуйтесь.", show_alert=True)
        else:
...
async def show_active_orders(message: Message, state: FSMContext):
    ...
    await _display_active_orders_list(message.bot, message.chat.id, user_role)


@router.callback_query(F.data.startswith(CB_PREFIX_COMPLETE_ORDER))
async def process_complete_order_callback(callback_query: CallbackQuery, state: FSMContext):
    ...
    if success:
        await callback_query.answer(f"Заказ #{order_id} выполнен!")
        await _display_active_orders_list(callback_query.bot, callback_query.message.chat.id, user_role,
...
async def edit_order_start(callback_query: CallbackQuery, state: FSMContext):
    ...
    if 'role' in current_data:
        await state.set_data({'role': current_data['role']})

    if callback_query.message:
        await _display_edit_order_interface(callback_query.message, order_id)
    await callback_query.answer()


@router.callback_query(F.data.startswith(CB_PREFIX_EDIT_ORDER_DELETE_PROMPT))
async def edit_order_delete_item_prompt(callback_query: CallbackQuery, state: FSMContext):
    if not await check_auth(callback_query, state): return
    try:
        order_id = int(callback_query.data.split(":")[1])
    except (IndexError, ValueError):
    ...
    if callback_query.message:
        await callback_query.message.edit_text(
            f"Какую позицию удалить из Заказа ID: {order_id}?",
...
async def edit_order_confirm_delete_item_action(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        order_item_id, order_id = int(parts[1]), int(parts[2])
    except (IndexError, ValueError):
        await callback_query.answer("Ошибка данных.", True);
        return

    if not await delete_order_item(order_item_id):
        await callback_query.answer("Не удалось удалить позицию.", True)
        if callback_query.message: await _display_edit_order_interface(callback_query.message, order_id)
        return

    remaining_items = await get_order_items(order_id)
    if not remaining_items:
        ...
        new_total = await recalculate_order_total_amount(order_id)
...
async def edit_order_finish(callback_query: CallbackQuery, state: FSMContext):
    user_role = await check_auth(callback_query, state)
    if not user_role: return
    ...
    await callback_query.bot.send_message(
        ...
        text="Вы вернулись к списку заказов.",
        reply_markup=menu_kb
    )
    await _display_active_orders_list(callback_query.bot, callback_query.from_user.id, user_role)


# --- ИЗМЕНЕНИЕ: Модифицируем этот хендлер ---
@router.callback_query(F.data.startswith(CB_PREFIX_EDIT_ORDER_ADD_ITEM_START))
async def edit_order_add_item_start_fsm(callback_query: CallbackQuery, state: FSMContext):
    if not await check_auth(callback_query, state): return
    ...
    order_data_check = await get_order_by_id(order_id)
    ...
    # Устанавливаем флаг и ID заказа
    await state.update_data(
        process_type="add_to_existing_order",
        editing_order_id=order_id,
        order_items=[],  # Инициализируем пустую "корзину" для новых товаров
        total_amount=0.0
    )
    ...
    await callback_query.bot.send_message(
        ...
        f"🛒 Добавление товара в заказ #{order_id}...\nВыберите категорию:",
