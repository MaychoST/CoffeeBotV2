...
async def cmd_help(message: types.Message, state: FSMContext):
    # Получаем данные о пользователе из состояния (FSM)
    ...
    if user_role == 'admin':
        ...
        # Для неавторизованных пользователей или если роль не найдена
        help_text += (
            "Добро пожаловать!\n"
...
# ЭТО ВЕРСИЯ, КОТОРАЯ НЕ ВОЗВРАЩАЕТ КЛАВИАТУРУ, НО РАБОТАЕТ
@router.message(BugReportStates.waiting_for_report_text, F.text)
async def bug_report_process_text(message: Message, state: FSMContext):
    data = await state.get_data()
    user_role = data.get('role')
    response_text = ""

    # Определяем, что делать - отменять или сохранять
    if message.text == GENERAL_CANCEL_TEXT:
        response_text = "Отправка отчета отменена."
    else:
        success = await save_bug_report(message.from_user.id, user_role, message.text)
        if success:
            response_text = "✅ Спасибо! Ваш отчет об ошибке отправлен."
        else:
            response_text = "❌ Не удалось отправить отчет. Попробуйте позже."

    # Шаг 1: Очищаем состояние FSM и восстанавливаем роль (безопасный паттерн)
    await state.clear()
    if user_role:
        await state.set_data({'role': user_role})

    # Шаг 2: Отправляем подтверждающее сообщение и убираем старую клавиатуру ("Отмена")
    await message.answer(response_text, reply_markup=ReplyKeyboardRemove())

    # Шаг 3 (РЕШЕНИЕ): Возвращаем пользователю его главное меню отдельным сообщением
    if user_role == 'admin':
